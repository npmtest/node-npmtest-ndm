...
<html lang="en">
<head>
    <title>Code coverage report for node-npmtest-ndm/node_modules/ndm/lib/config.js</title>
    <meta charset="utf-8">
    <style>
        body, html {
            margin:0; padding: 0;
...
<div class="header medium">
    ...
    <h1>Code coverage report for <span class="entity">node-npmtest-ndm/node_modules/ndm/lib/config.js</span></h1>
    <h2>
        
        Statements: <span class="metric">72.92% <small>(35 / 48)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
    ...
    <div class="path"><a href="../../../../index.html">All files</a> &#187; <a href="index.html">node-npmtest-ndm/node_modules/ndm/lib/</a> &#187; config.js</div>
</div>
<div class="body">
...
function Config(opts) {
  _.extend(this, {
    ...
    parsers: {
      sudo: <span class="fstat-no" title="function not covered" >function(v) {</span> <span class="cstat-no" title="statement not covered" >return v.toString() === 'true' }</span>,
      headless: <span class="fstat-no" title="function not covered" >function(v) {</span> <span class="cstat-no" title="statement not covered" >return v.toString() === 'true' }</span>,
      globalPackage: <span class="fstat-no" title="function not covered" >function(v) {</span> <span class="cstat-no" title="statement not covered" >return v.toString() === 'true' }</span>
    }
  }, opts);
&nbsp;
...
Config.prototype.updateWithOSDefaults = <span class="fstat-no" title="function not covered" >function() {</span>
<span class="cstat-no" title="statement not covered" >  var _this = this,</span>
    osDefaults = this._getOSDefaults();
&nbsp;
<span class="cstat-no" title="statement not covered" >  Object.keys(osDefaults).forEach(<span class="fstat-no" title="function not covered" >function(key) {</span></span>
<span class="cstat-no" title="statement not covered" >    if (!_this._argv || !_this._argv[key]) <span class="cstat-no" title="statement not covered" >_this[key] = osDefaults[key];</span></span>
  });
}
&nbsp;
...
&nbsp;
  Object.keys(ndmrc).forEach(function(key) {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (typeof _this[key] !== 'function') {
      _this[key] = _this.parsers[key] ? <span class="branch-0 cbranch-no" title="branch not covered" >_this.parsers[key](ndmrc[key]) </span>: ndmrc[key];
    }
  });
};
&nbsp;
...
Config.prototype._getOSDefaults = <span class="fstat-no" title="function not covered" >function() {</span>
  // default to Ubuntu OS settings.
<span class="cstat-no" title="statement not covered" >  return (this.platformApis[this.platform] || this.platformApis['ubuntu']).configOverrides;</span>
};
&nbsp;
...
&nbsp;
  Object.keys(this.env).forEach(function(key) {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (key.match(/^NDM_/)) {
<span class="cstat-no" title="statement not covered" >      opts[S(key.replace(/^NDM_/, '').toLowerCase()).camelize().s] = _this.env[key];</span>
    }
  });
&nbsp;
  return opts;
...
Config.prototype.os = function() {
  var _this = this;
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (this.platform) <span class="cstat-no" title="statement not covered" >return this.platform;</span>
  else {
    Object.keys(this.platformApis).forEach(function(key) {
      var api = new _this.platformApis[key]();
      <span class="missing-if-branch" title="if path not taken" >I</span>if (api.isPlatform()) <span class="cstat-no" title="statement not covered" >_this.platform = api.platform;</span>
    });
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.platform) <span class="cstat-no" title="statement not covered" >return this.platform;</span>
  }
...
module.exports = function(opts, resetConfig) {
  ...
  else <span class="missing-if-branch" title="if path not taken" >I</span>if (opts) <span class="cstat-no" title="statement not covered" >_.extend(config, opts);</span>
&nbsp;
  return config;
};
&nbsp;
